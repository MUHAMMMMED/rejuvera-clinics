...
def create_default_modules(sender, instance, created, **kwargs):
    if created:
        ...
        Feature.objects.get_or_create(
            service=instance,
            defaults={
                'title': f'خدمة {instance.name}',
                'subtitle': 'نقدم لك تجربة تجميلية استثنائية بأحدث التقنيات وأعلى معايير الجودة',
                'results_title': 'نتائج فورية',
                'results_description': 'ترى الفرق مباشرة بعد الجلسة',
                'safety_title': 'آمن تماماً',
                'safety_description': 'تقنيات معتمدة دولياً',
                'recovery_title': 'تعافي سريع',
                'recovery_description': 'عودي لحياتك الطبيعية خلال أيام',
                'care_title': 'رعاية شاملة',
                'care_description': 'متابعة مستمرة بعد الإجراء'
            }
        )

        # ProcessSteps افتراضي
        ProcessSteps.objects.get_or_create(
            service=instance,
            defaults={
                'title': 'رحلة تحولك خطوة بخطوة',
                'subtitle': 'نرافقك في كل مرحلة لضمان أفضل النتائج',
                'consultation_title': 'استشارة مجانية',
                'consultation_description': 'تقييم شامل ووضع خطة مخصصة',
                'consultation_duration': '30-45 دقيقة',
                'preparation_title': 'تحضير مسبق',
                'preparation_description': 'فحوصات طبية وتجهيز المنطقة',
                'preparation_duration': '15-30 دقيقة',
                'procedure_title': 'الإجراء',
                'procedure_description': f'خدمة {instance.name} بأحدث التقنيات',
                'procedure_duration': '60-180 دقيقة',
                'followup_title': 'متابعة ونتائج',
                'followup_description': 'رعاية متكاملة ونتائج مذهلة',
                'followup_duration': 'متابعة مستمرة'
            }
        )

        # ServiceHero افتراضي
        ServiceHero.objects.get_or_create(
            service=instance,
            defaults={
                'title': instance.name,
                'subtitle': 'استعيدي ثقتك بجسم مثالي متناسق',
                'description': f'خدمة {instance.name} مع أفضل التقنيات والأجهزة الحديثة',
                'badge': 'أحدث التقنيات',
                'cta_text': 'احجزي استشارتك المجانية',
            }
        )

        # ServiceTrust افتراضي
        ServiceTrust.objects.get_or_create(
            service=instance,
            defaults={
                'experience_years': '+10',
                'success_operations': '+500',
                'doctors_count': '+20',
                'satisfaction_rate': '98%'
            }
        )

        # ServiceProblemSolution افتراضي
        ServiceProblemSolution.objects.get_or_create(
            service=instance,
            defaults={
                'problem_title': f'التحديات في {instance.name}',
                'problem_description': 'وصف المشكلة بشكل مبسط لتوضيح الحاجة لهذه الخدمة',
                'solution_title': f'حلول {instance.name}',
                'solution_description': 'شرح كيف تساعد هذه الخدمة في حل المشكلة وتحقيق أفضل النتائج'
            }
        )